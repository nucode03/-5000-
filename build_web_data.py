...
def korean_pos(source_detail: str) -> str:
    ...
    tokens = re.findall(r"n\.|v\.|adj\.|adv\.|prep\.|conj\.|pron\.|det\.|exclam\.|number", cleaned)
    if not tokens:
...
def main() -> None:
    ...
    words = [
        {
            ...
            "meaning": row["meaning"],
            "letter": row["word"][0].upper(),
        }
        for index, row in enumerate(source_rows, start=1)
    ]
    if len(words) != 2000 or len({word["id"] for word in words}) != len(words):
        raise ValueError("Expected 2,000 uniquely identified entries.")
    ...
    DESTINATION.write_text(json.dumps(words, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
